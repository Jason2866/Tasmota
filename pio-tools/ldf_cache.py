...
def capture_complete_scons_environment():
    """Erfasst nur compile-relevante SCons-Environment Variablen"""
    
    print(f"\n🎯 COMPILE-RELEVANTE SCons-Environment-Erfassung:")
    
    # 1. Erweiterte LDF-Variablen erfassen (mit rekursiver lib-Sicherung)
    ldf_variables = export_ldf_variables_extended()
    
    # 2. NUR compile-relevante SCons-Variablen erfassen
    compile_relevant_vars = [
        'LIBS', 'ASFLAGS', 'ASPPFLAGS', 'CFLAGS', 'CXXFLAGS', 
        'CCFLAGS', 'LINKFLAGS', 'CPPPATH', 'LIBPATH', 'CPPDEFINES'
    ]
    
    scons_data = {}
    conversion_stats = {"file_paths": 0, "builders": 0, "functions": 0, "clvar_converted": 0, "other": 0}
    
    for var in compile_relevant_vars:
    ...
    print(f"   ✅ Nur compile-relevante Variablen gesichert: {len(compile_relevant_vars)}")
...
def freeze_complete_scons_configuration(complete_data):
    ...
    try:
        with open(temp_file, 'w', encoding='utf-8') as f:
            ...
            f.write('    \n')
            f.write('    # Andere compile-relevante Variablen - NUR compile-relevante\n')
            f.write('    compile_relevant_vars = ["LIBS", "ASFLAGS", "ASPPFLAGS", "CFLAGS", "CXXFLAGS", "CCFLAGS", "LINKFLAGS", "LIBPATH", "CPPDEFINES"]\n')
            f.write('    for var in compile_relevant_vars:\n')
            f.write('        if var in SCONS_VARS:\n')
            f.write('            try:\n')
            f.write('                # env.replace für vollständige Ersetzung\n')
            f.write('                target_env.Replace(**{var: SCONS_VARS[var]})\n')
            f.write('                critical_restored += 1\n')
            f.write('                print(f"   {var} wiederhergestellt (replace)")\n')
            f.write('            except Exception as e:\n')
            f.write('                try:\n')
            f.write('                    # Fallback: env.append für Erweiterung\n')
            f.write('                    target_env.Append(**{var: SCONS_VARS[var]})\n')
            f.write('                    critical_restored += 1\n')
            f.write('                    print(f"   {var} wiederhergestellt (append)")\n')
            f.write('                except Exception as e2:\n')
            f.write('                    print(f"   Fehler bei {var}: {e} / {e2}")\n')
            f.write('    \n')
            ...
            f.write('    """Analysiert Unterschiede zwischen CPPPATH-Quellen"""\n')
            f.write('    sources = get_cpppath_sources()\n')
            f.write('    complete = get_complete_cpppath()\n')
            f.write('    \n')
            f.write('    print("CPPPATH-Quellen-Analyse:")\n')
            f.write('    for source_name, paths in sources.items():\n')
            f.write('        if isinstance(paths, list):\n')
            f.write('            print(f"  {source_name}: {len(paths)} Pfade")\n')
            f.write('            for i, path in enumerate(paths[:3]):\n')
            f.write('                print(f"    {i}: {path}")\n')
            f.write('            if len(paths) > 3:\n')
            f.write('                print(f"    ... und {len(paths) - 3} weitere")\n')
            f.write('    \n')
            f.write('    print(f"Vollständige CPPPATH: {len(complete)} Einträge")\n')
            f.write('    return sources\n\n')
            
            f.write('# Für Debugging\n')
            f.write('if __name__ == "__main__":\n')
            f.write('    print("LDF Cache File - Debug Informationen")\n')
            f.write('    print(f"SCons Vars: {list(SCONS_VARS.keys())}")\n')
            f.write('    print(f"LDF Vars: {list(LDF_VARS.keys())}")\n')
            f.write('    analyze_cpppath_diff()\n')
        
        # Atomarer Dateiaustausch
        if os.path.exists(cache_file):
            backup_cache = cache_file + ".old"
            shutil.move(cache_file, backup_cache)
        
        shutil.move(temp_file, cache_file)
        
        print(f"✅ SCons-Environment-Cache gespeichert: {os.path.basename(cache_file)}")
        print(f"   📊 Compile-relevante Variablen: {len(complete_data['SCONS_VARS'])}")
        print(f"   📁 CPPPATH-Einträge: {len(complete_data['LDF_VARS']['LIB_VARS']['CPPPATH_COMPLETE'])}")
        print(f"   🔄 Rekursive lib-Erfassung: Aktiviert")
        
        return True
        
    except Exception as e:
        print(f"❌ Fehler beim Speichern der SCons-Environment: {e}")
