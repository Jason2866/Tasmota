...
#include <Arduino.h>
#ifdef ESP32
// includes done in "AudioOutputI2S.h"
...
bool AudioOutputI2S::SetRate(int hz)
{
  ...
  #else
      std_cfg.clk_cfg  = I2S_STD_CLK_DEFAULT_CONFIG(hertz);
  #endif
  #elif defined(ESP8266)
...
bool AudioOutputI2S::begin(bool txDAC)
{
    ...
    std_cfg.clk_cfg  = I2S_STD_CLK_DEFAULT_CONFIG(hertz);
    std_cfg.slot_cfg = I2S_STD_MSB_SLOT_DEFAULT_CONFIG(static_cast<i2s_data_bit_width_t>(bps), static_cast<i2s_slot_mode_t>(channels));
    ...
    std_cfg.gpio_cfg.din  = (gpio_num_t)I2S_PIN_NO_CHANGE;
